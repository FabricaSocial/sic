...
class POSprinter:
    ...
    def __init__(self, port="/dev/ttyUSB0", baudrate=9600, bytesize=8, parity='N', stopbits=1, charWidth=44, pxWidth=284):
        ...
        # Multiple inheritance may become a nightmare, so we are importing the
        # modules insted.
        try:
            import serial
        except:
    ...
    def write(self, string, rcolStr=None, align="left"):
        ...
        if not rcolStr:
            ...
            if "\n" in string or "\n" in rcolStrRstripNewline:
                raise ValueError(
                    """When using rcolStr in POSprinter.write only newline at
                    the end of rcolStr is allowed and not in string (the main
                    text string) it self.""")
            # expand string
            ...
            if lastLineLen > self.width:
                ...
                string += numOfBlanks * " "
                lastLineLen = len(string) % self.width + len(
                    rcolStrRstripNewline)
            if lastLineLen < self.width:
    ...
    def printImgFromPILObject(self, imgObject, resolution="high", align="center", scale=None):
        ...
        try:
            ...
            if scale:
                ...
                imgObjectB = imgObject.resize(
                    [int(scaleTuple[i] * imgObject.size[i]) for i in range(2)]).convert("1")
            else:
    ...
    def printImgMatrix(self, imgMatrix, width, height, resolution, align):
        ...
        if width > currentpxWidth:
            raise ValueError("Image too wide. Maximum width is configured to be " + str(
                currentpxWidth) + "pixels. The image is " + str(width) + " pixels wide.")
        for yScale in range(-(-height / scaling)):
            ...
            # Add width to the communication to the printer. Depending on the
            # alignment we count that in and add blank vertical lines to the
            # outList
            if align == "left":
                blanks = 0
            if align == "center":
            ...
            for x in range(width):
                # Compute hex string for one vertical bar of 8 dots (zero
                # padded from the bottom if necessary).
                binStr = ""
                for y in range(scaling):
                    # Indirect zero padding. Do not try to extract values from
                    # images beyond its size.
                    if (yScale * scaling + y) < height:
                        binStr += "0" if imgMatrix[
                            x, yScale * scaling + y] == 255 else "1"
                    # Zero padding
                    else:
                        binStr += "0"
